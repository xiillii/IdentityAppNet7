...
public class UserTwoFactorSetupModel : UserPageModel
{
    ...
    public UserTwoFactorSetupModel(UserManager<IdentityUser> usrMgr, SignInManager<IdentityUser> signMgr)
    {
        UserManager = usrMgr;
    ...
    public async Task<IActionResult> OnPostConfirm([Required] string confirm)
    {
        ...
        if (ModelState.IsValid)
        {
            ...
            var codeValid = await UserManager.VerifyTwoFactorTokenAsync(IdentityUser,
    ...
    private async Task LoadAuthenticatorKeys()
    {
        ...
        QrCodeUrl = $"otpath://totp/ExampleApp:{IdentityUser.Email}?secret={AuthenticatorKey}";
    }
}